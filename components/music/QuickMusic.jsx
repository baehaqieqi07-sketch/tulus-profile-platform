...
export default function QuickMusic({ profile, setProfile }) {
  ...
  const pickFile = (file) => {
    ...
    setProfile((prev) => ({ ...prev, music_source_type: 'upload', music_upload_url: url, music_url: url, music_title: prev.music_title || file.name.replace(/\.(mp3|ogg|wav|m4a|aac|flac)$/i, ''), show_music: true }))
  }
  return (
