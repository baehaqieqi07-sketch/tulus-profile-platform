...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { user_id, plan, status = 'active' } = await req.json()
    if (!user_id || !['free','plus','pro','lifetime'].includes(plan)) return json({ ok: false, message: 'Valid user and plan are required.' }, 400)
    const { data, error } = await admin.from('subscriptions').upsert({ user_id, plan, status }, { onConflict: 'user_id' }).select('*').single()
    if (error) throw error
    await logAction(admin, user.id, role, 'update_user_plan', 'user', user_id, { plan, status })
    return json({ ok: true, subscription: data })
  } catch (e) {
    return json({ ok: false, message: e.message === 'not_found' ? 'page not found' : 'Could not update plan.' }, e.message === 'not_found' ? 404 : 400)
  }
})
