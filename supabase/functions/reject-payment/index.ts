...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: payment, error } = await admin.from('payments').update({ status: 'rejected' }).eq('id', payment_id).select('*').single()
    if (error) throw error
    await logAction(admin, user.id, role, 'reject_payment', 'payment', payment_id, { reason })
    return json({ ok: true, payment })
  } catch (e) {
    return json({ ok: false, message: e.message === 'not_found' ? 'page not found' : 'Could not reject payment.' }, e.message === 'not_found' ? 404 : 400)
  }
})
