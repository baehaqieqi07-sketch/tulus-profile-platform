...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await admin.from('profiles').update({ is_suspended: false }).eq('id', profile_id).select('*').single()
    if (error) throw error
    await logAction(admin, user.id, role, 'unsuspend_profile', 'profile', profile_id)
    return json({ ok: true, profile: data })
  } catch (e) {
    return json({ ok: false, message: e.message === 'not_found' ? 'page not found' : 'Could not unsuspend profile.' }, e.message === 'not_found' ? 404 : 400)
  }
})
